...
const addParticipant = async (req, res) => {
  try {
    ...
    if (chat.participants.find(p => String(p.participantId) === String(participant._id))) {
      return res.status(400).json({
...
const removeParticipant = async (req, res) => {
  try {
    ...
    const newParticipants = chat.participants.filter(p => String(p.participantId) !== String(participant._id))
    await Chat.findByIdAndUpdate(chat._id, {
      participants: newParticipants
    })
